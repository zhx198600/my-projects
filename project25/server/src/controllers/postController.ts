...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    const fakeSensitiveCheck = (text: string) => {
    ...
    try {
      post = new Post({
        ...
        content,
        category,
        author: new mongoose.Types.ObjectId((req.user as any).id),
      });
      await post.save();
      ...
      return res.status(201).json({
        ...
        post: {
          ...
          content,
          category,
          author: (req.user as any) || { _id: 'demo', username: '演示用户' },
...
export let demoCommentsStore: any[] = [
  {
    _id: 'c1',
...
export const flattenComments = (comments: any[]): any[] => {
  let result: any[] = [];
  comments.forEach((c: any) => {
    result.push({ ...c, children: undefined });
    if (c.children && c.children.length > 0) {
      result = result.concat(flattenComments(c.children));
    }
  });
  return result;
};
...
export const getPosts = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      let filteredPosts = [...demoPostsStore];
      if (category && category !== '全部') {
        filteredPosts = demoPostsStore.filter((p) => p.category === category);
      }
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const demoPost = demoPostsStore.find((p) => p._id === req.params.id) || demoPostsStore[0];
...
export const getPostComments = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const postComments = demoCommentsStore.filter((c) => c.postId === req.params.id && !c.parentId);
